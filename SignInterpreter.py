...
def mediapipe_points(image, model):
...
#     while vd.isOpened():
#       ret, frame = vd.read()
#       image, results = mediapipe_points(frame, holistics)
#       mediapipe_draw(frame, results)
...
actions = np.array(['arrival', 'departure', 'india', 'japan'])
no_seq = 30 
seq_len = 30 
...
def prob_viz(res, actions, input_frame, colors):
    ...
    # Determine the minimum length to avoid index out of range error
    min_length = min(len(res), len(actions), len(colors))
    
    for num in range(min_length):
        prob = res[num]
        action_text = f"{actions[num]}: {prob:.2f}"
        cv.rectangle(output_frame, (0, 60 + num * 40), (int(prob * 100), 90 + num * 60), colors[num], -1)
        cv.putText(output_frame, action_text, (0, 85 + num * 60), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv.LINE_AA)
...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    while cap.isOpened():
        ...
        # Make detections
        image, results = mediapipe_points(frame, holistic)
        print(results)
